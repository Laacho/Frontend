...
const ROLE_COLORS: Record<string, { bg: string; fg: string }> = {
  USER: { bg: 'var(--c-surface-2)', fg: 'var(--c-text-2)' },
  STAFF: { bg: '#FBF1E2', fg: '#B5781E' },
  ADMIN: { bg: '#FBEDEB', fg: '#A8362F' },
};

function RoleBadge({ role }: { role: string }) {
  const c = ROLE_COLORS[role] || { bg: 'var(--c-surface-2)', fg: 'var(--c-text-2)' };
  return (
    <span
    ...
    <div className="animate-bub-fade">
      ...
      <div className="flex gap-3 mb-5">
        <div className="relative flex-1 max-w-xs">
          <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 pointer-events-none" style={{ color: 'var(--c-text-muted)' }} viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5">
        ...
        <div className="flex gap-2">
          {['ALL', 'USER', 'STAFF', 'ADMIN'].map(role => (
            <button
              ...
              style={{
                borderColor: roleFilter === role ? '#0F2A47' : 'var(--c-border)',
                backgroundColor: roleFilter === role ? '#0F2A47' : 'var(--c-surface)',
                color: roleFilter === role ? 'var(--c-surface)' : 'var(--c-text-2)',
          ...
          {filtered.map(user => (
            <div
              ...
              style={{
                ...
                borderColor: selectedUser?.id === user.id ? '#0F2A47' : 'var(--c-border)',
              }}
            >
              {/* Avatar */}
              <div
                ...
                style={{
                  backgroundColor: user.role === 'ADMIN' ? '#0F2A47' : user.role === 'STAFF' ? '#C8A878' : 'var(--c-bg)',
                  color: user.role === 'ADMIN' ? '#C8A878' : user.role === 'STAFF' ? '#0F2A47' : 'var(--c-text-2)',
        ...
        <div className="lg:col-span-1">
          {selectedUser ? (
            <div className="rounded-xl border p-5 sticky top-6" style={{ backgroundColor: 'var(--c-surface)', borderColor: 'var(--c-border)' }}>
              <div className="flex items-center gap-3 mb-5 pb-5 border-b" style={{ borderColor: 'var(--c-surface-2)' }}>
                <div
                  className="w-12 h-12 rounded-full flex items-center justify-center text-base font-semibold"
                  style={{
                    backgroundColor: selectedUser.role === 'ADMIN' ? '#0F2A47' : selectedUser.role === 'STAFF' ? '#C8A878' : 'var(--c-bg)',
                    color: selectedUser.role === 'ADMIN' ? '#C8A878' : selectedUser.role === 'STAFF' ? '#0F2A47' : 'var(--c-text-2)',
                    fontFamily: 'Geist, sans-serif',
                  }}
                >
              ...
              <div className="space-y-2 mb-5">
                {[
                  ...
                  <div key={row.label} className="flex justify-between py-2 border-b" style={{ borderColor: 'var(--c-surface-2)' }}>
                    ...
                    <span className="text-xs font-medium" style={{ color: 'var(--c-text)', fontFamily: row.mono ? '"Geist Mono", monospace' : 'Geist, sans-serif' }}>
                      {row.value}
                    </span>
                  </div>
                ))}
            ...
            >
              <p className="text-sm" style={{ color: 'var(--c-text-muted)', fontFamily: 'Geist, sans-serif' }}>
